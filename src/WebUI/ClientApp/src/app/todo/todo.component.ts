...
export class TodoComponent implements OnInit {
    ...
    remainingItems(list: TodoListDto): number {
        return list.items.filter(t => !t.done && t.isActive === true).length;
    }
    ...
    softDeleteListConfirmed(): void {
        const list = this.listOptionsEditor as UpdateTodoListCommand;
        this.listOptionsEditor.isActive = false;
        this.listsClient.update(this.selectedList.id, list).subscribe(
            () => {
    ...
    addItem() {
        ...
        const remaining = this.remainingItems(this.selectedList);
    ...
    softDeleteItem(): void {
        ...
        this.itemsClient.updateItemDetails(this.selectedItem.id, item).subscribe(
            () => {
                this.selectedList.isActive = item.isActive;
                (this.selectedList.items = this.selectedList.items.filter(
                    t => t.id !== item.id
                )),
                    this.itemDetailsModalRef.hide();
